...
describe("Wellbeing Test routes test", () => {
    test("get test questions (SLS)", async () => {
        const response = await (0, supertest_1.default)(BASE_URL).get("/wellbeing/questions/SLS");
        expect(response.status).toEqual(200);
    });
    test("get test when param doesnt match enum", async () => {
        const response = await (0, supertest_1.default)(BASE_URL).get("/wellbeing/questions/test");
    ...
    test("get user test history", async () => {
        const response = await (0, supertest_1.default)(BASE_URL).get("/wellbeing/history?id=e8649e0f-0051-4c8c-b603-7e9834544ccb");
        expect(response.status).toEqual(200);
    });
});
